...
- 点击任一点位可以查看并复制经纬度。
- 点位按最近铁塔距离显示：0–250 米（含 250）绿色，>250–500 米（含 500）蓝色，>500 米红色；弹窗显示具体距离。
- 本地开发快照保存在 `archive/tonghai-railway-local-20260825/`，可独立运行和继续刷新坐标。

距离数据来自 `../通海铁路沿线.xls` 的“最近铁塔距离（米）”列。修改表格后，在本目录重新导入并构建：

```bash
python3 -m pip install -r tools/requirements-map.txt
python3 tools/import-tower-distances.py ../通海铁路沿线.xls
npm run build
```

导入按“序号”匹配，并核对现有地图坐标；兼容该表中写反的经纬度列标题，不改变点位坐标。表格中的汇总行不参与导入。刷新坐标数据后也需重新导入距离。
